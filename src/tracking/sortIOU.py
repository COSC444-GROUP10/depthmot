...
def convert_bbox_to_z(bbox):
    ...
    - s is the scale (area of the bounding box - width * height)
    ...
    y = bbox[1] + h/2.
    z = bbox[2] + d/2.
    s = w * h  # scale is area of the box (width * height)
...
def convert_x_to_bbox(x,score=None):
    ...
    # Calculate width and height from area (scale) and aspect ratio
    # Since scale = w * h and ratio = w / h
    # We get w = sqrt(scale * ratio) and h = sqrt(scale / ratio)
    width = np.sqrt(scale * ratio)
    height = np.sqrt(scale / ratio)
...
if __name__ == "__main__":
    ...
    mot_tracker = SortIOU(max_age=1, min_hits=1, iou_threshold=0.3)
