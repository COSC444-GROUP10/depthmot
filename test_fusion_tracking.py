...
def test_video_with_tracking():
    ...
    try:
        ...
        while cap.isOpened():
            ...
            # Convert descriptors to SORT format 
            sort_dets = fusion.get_sort_detections(descriptors)
            
            # Update SORT tracker (returns tracks with [xmin, ymin, zmin, xmax, ymax, zmax, track_id])
            ...
            # Display number of detections and tracks
            det_text = f"Detections: {len(descriptors)} Tracks: {len(tracks)}"
            cv2.putText(vis_frame, det_text, (10, 110), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
            
            # Print progress every 10 frames
            if frame_count % 10 == 0:
                print(f"Processing frame {frame_count}/{total_frames}, {len(descriptors)} detections, {len(tracks)} tracks, {1.0/processing_time:.2f} FPS")
